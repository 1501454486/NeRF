...
class AELoss(nn.Module):
    ...
    def forward(self, ae, ind, ind_mask):
        ...
        push = (
            (push_dist.sum(dim=(1, 2)) - obj_num) / (obj_num * (obj_num - 1) + 1e-4)
        ).sum()
        push /= b
        return pull, push
...
class PolyMatchingLoss(nn.Module):
    def __init__(self, pnum):
        ...
        pidxall = torch.from_numpy(np.reshape(pidxall, newshape=(batch_size, -1))).to(
            device
        )

        self.feature_id = (
            pidxall.unsqueeze_(2)
            .long()
            .expand(pidxall.size(0), pidxall.size(1), 2)
            .detach()
        )
...
class Ind2dRegL1Loss(nn.Module):
    ...
    def forward(self, output, target, ind, ind_mask):
        ...
        pred = _tranpose_and_gather_feat(output, ind).view(
            b, max_objs, max_parts, output.size(1)
        )
...
class GeoCrossEntropyLoss(nn.Module):
    ...
    def forward(self, output, target, poly):
        ...
        target = target[..., None, None].expand(
            poly.size(0), poly.size(1), 1, poly.size(3)
        )
...
def load_pretrain(net, model_dir):
    ...
    print(
        "Load pretrain model: {}".format(os.path.join(model_dir, "{}.pth".format(pth)))
    )
    pretrained_model = torch.load(os.path.join(model_dir, "{}.pth".format(pth)), "cpu")
    net.load_state_dict(pretrained_model["net"])
    return 0
